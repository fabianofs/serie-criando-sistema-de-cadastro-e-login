...
var installUrl = $('script[src$="jquery.godsplayer.js"]').attr('src');
...
GodsPlayer.prototype.refreshArtwork = function() {  
    var me = this;
    $.ajax({
...
GodsPlayer.prototype.showMessage = function(title, subtitle) {
      var me = this;
      var search_term = title + ' ' + subtitle;
      if(search_term == me._old_search_term) return;
      me._old_search_term = search_term;
      if(me.display_hnd != null) {
        clearTimeout(me.display_hnd);
        me.display_hnd = null;
        if(!title)
          title = me.config.radioName == undefined? '' : me.config.radioName;
        me.title.html(title).fadeIn(0);
        me.subtitle.html(subtitle).fadeIn(0);
      } else {
        me.title.fadeOut(300);
        me.subtitle.fadeOut(250);

        function delayedUpdate() {
          if(!title)
            title = me.config.radioName == undefined? '' : me.config.radioName;
          me.title.html(title).fadeIn(500);
          me.subtitle.html(subtitle).fadeIn(500);
          me.display_hnd = null;
        }
        me.display_hnd = setTimeout(delayedUpdate,301);
      }
}

GodsPlayer.prototype.updateDisplay = function() {
  var me = this;
  setTimeout(function() {me.updateDisplay();}, me.config.streamTxtRefreshInterval*1000);
  function updateTitlesHtml(text) {
      var title = '';
      var subtitle = '';
      if(text) {
        var g = text.match(/(<p>)?(.*,.*,.*,.*,.*,.*,|.*title=)([^-]*)?([ -]*)?([^<]*)?(<\/p>)?/);
        deb('HTML REGEX = '+g);
        if(g[3] != undefined)
          title = g[3];
        if(g[5] != undefined)
      ...
      me.showMessage(title, subtitle);
      if(me.cover != null)
        me.refreshArtwork();
  }
  deb('******** getting title: '+me.config.streamTxt);
  $.get(me.config.streamTxt, function(res) {
    deb('UpdateTitles '+me.config.streamTxt+': '+res);
    updateTitlesHtml(res);
  }, 'html');
}
